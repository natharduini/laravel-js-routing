	...
	public static function generate() {
		...
		foreach ($router as $route) {
			if ($route->getActionName() != 'Closure') {
				$routesByAction[$route->getActionName()]['route'] = str_replace('?}', '}', Config::get('app.url') . $route->getPath());
				$routesByAction[$route->getActionName()]['parameters'] = $route->parameterNames();
			}
			if ($route->getName() != '') {
				$routesByName[$route->getName()]['route'] = str_replace('?}', '}', Config::get('app.url') . $route->getPath());
